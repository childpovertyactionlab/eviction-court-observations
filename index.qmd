...
Coverage was greatest in `r casesByCourtDeac %>% slice_max(obsCases, n = 1) %>% pull(jpCourt)`: observers recorded data for 2,349
 cases, representing `r scales::percent(max(jpCasesDeac$perAllCases), accuracy = .1)` coverage. Although observers were in nine courts during the observation period, data from two courts (4-2 and 5-2) are not reported here due to a limited sample of observed cases.
...
:::
<br>
**Landlords or their representatives almost always appeared for the hearing, but tenants appeared about half of the time. However, tenant appearance has improved (+14% points) over previous observation cycles.**
...
:::
<br>
**Legal representation of tenants has improved (+25% points) over previous observation cycles. **
...
highchart()%>%
  ...
  hc_yAxis(title = list(text = ""), labels = list(format = "{value}%"), max = 100) %>%
  hc_tooltip(pointFormat = "Percentage of Cases with Discussed NTV: {point.y:.2f}%") %>%
  #hc_colors(palette_cpal_main)%>%
  hc_add_theme(hc_cpal)
