...
`r format(as.Date("2024-04-26"), "%B %d, %Y")`, `r smuStudents` SMU Law student
...
## Across Summer '23 - Spring '24 observed cases, we found:
...
## Across all cases in the Summer '23 - Spring '24 Cycle, the average length of time for each hearing was less than 4 minutes.
...
drilldown_data <- caseOutcome %>%
  ...
  mutate(
    ...
    data = map(data, function(df) {
      list_parse2(df %>% select(monthYear, n))
    })
  )
...
highchart()%>%
  hc_chart(type='line')%>%
  hc_add_series(data = caseOutcomeTotal,
                type = 'line',
                hcaes(x = monthYear,
                      y = n,
                      group = outcomeParty),
                color = outcomeParty_colors)%>%
  hc_xAxis(type = 'category')%>%
  hc_add_theme(hc_cpal)%>%
  hc_colors(outcomeParty_colors) %>% 
