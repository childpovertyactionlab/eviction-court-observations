...
deac <- read_sheet("https://docs.google.com/spreadsheets/d/11X91mVG7XSTDk3fjb8gCiyXDToh80yyvrVdLJiZOpOw/edit?gid=480254842#gid=480254842", col_types = "c") %>%
  rename(caseNumber = case_number,
         observationDate = court_date,
  ...
  mutate(jpCourt = paste("Court", jpCourt),
         ...
         observationDate = if_else(
           ...
           observationDate)
         # totalOwned = as.numeric(totalOwned)
         )
...
# deacCourt11 <- deac %>%
#   filter(jpCourt == "Court 1-1")
# evicCourt11 <- evictions %>%
#   filter(jpCourt == "Court 1-1",
#          appearance_date >= as.Date("2024-09-26") & appearance_date <= as.Date("2025-01-18"))
...
caseAppearDeac <- rbind(defAppear24, plaAppear24)
...
ntvDeac <- deac %>%
  ...
  bind_rows() %>%
  mutate(test = lag(noticeVacate),
         noticeVacate = ifelse(str_detect(noticeVacate, "Y|N"), noticeVacate, test)) %>%
  select(-test) %>%
  group_by(jpCourt) %>%
  mutate(perntv = count/sum(count)) %>%
  ungroup()
...
caseOutcomeDeac <- deac %>%
  ...
  mutate(outcomeParty = ifelse(is.na(jpRuled), "Unknown",
                               ifelse(str_detect(jpRuled, "Plaintiff"), "Ruled in Favor of Landlord",
                                      ifelse(str_detect(jpRuled, "Defendant"), "Ruled in Favor of Tenant", 
                                             ifelse(str_detect(jpRuled, "abated"), "Abated",
                                                    ifelse(str_detect(jpRuled, "dismissed"), "Dismissed", jpRuled)))))) %>%
  group_by(jpCourt, outcomeParty) %>%
...
totevicDeac <- evictions%>%
  filter(filed_date >= as.Date('2024-04-26') & filed_date < as.Date('2025-01-18'))
startDate <- min(deac$observationDate, na.rm = TRUE)
endDate <- max(deac$observationDate, na.rm = TRUE)

#total days between start and end of data
totdaysDeac <- difftime(endDate, startDate, "days") %>%
  as.numeric()
...
for (court in unique(evicReasonDeac$jpCourt)){
  court_data <- evicReasonDeac %>% filter(jpCourt == court)%>%
...
```{r}
#| fig-height: 5
plotAppearDeac <- caseAppearDeac%>%
  mutate(AppearPer=round(AppearPer*100, 2))
highchart()%>%
  hc_chart(type = 'column')%>%
  hc_add_series(plotAppearDeac,
                type = 'column',
                mapping = hcaes(
                  x = jpCourt,
                  y = AppearPer,
                  group = type,
                  color = ifelse(jpCourt == "Total", 
                                 c('#4B1707','#ED683F')[match(type, unique(plotAppearDeac$type))], 
                                 palette_cpal_main[match(type, unique(plotAppearDeac$type))])),
                color = c('#008097', '#042d33'),
                dataLabels = list(enabled = TRUE, format = "{point.y:.0f}%", style = list(fontWeight = "bold")))%>%
  hc_xAxis(categories = plotAppearDeac$jpCourt) %>%
  hc_yAxis(labels = list(format = "{value: .0f}%"), 
           max = 100) %>%
  hc_plotOptions(column = list(
    dataLabels = list(enabled = TRUE, format = "{value:.0f}%")
...
plotDefDeac <- defRepresentDeac%>%
  ...
  add_row(., tibble(jpCourt = "Court 3-1", legalRep = "Unknown", count = 0, perRep = 0))%>%
  add_row(., tibble(jpCourt = "Court 3-2", legalRep = "Unknown", count = 0, perRep = 0))%>%
  add_row(., tibble(jpCourt = "Court 2-1", legalRep = "No attorney present for tenant", count = 0, perRep = 0))%>%
  add_row(., tibble(jpCourt = "Court 2-2", legalRep = "No attorney present for tenant", count = 0, perRep = 0))%>%
  add_row(., tibble(jpCourt = "Court 4-2", legalRep = "Attorney present for tenant (Defendant)", count = 0, perRep = 0))%>%
  add_row(., tibble(jpCourt = "Court 4-2", legalRep = "No attorney present for tenant", count = 0, perRep = 0))%>%
  mutate(legalRep = case_when(is.na(legalRep) ~ "Unknown",
                              TRUE ~ legalRep)) %>%
  group_by(jpCourt, legalRep)%>%
...
```{r}
#| fig-height: 4
court_order <- c("Court 1-1", "Court 1-2", "Court 3-1", "Court 4-2", "Court 5-1", "Total")
