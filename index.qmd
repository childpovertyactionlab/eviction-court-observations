...
ntvDeacObserver <- deac %>%
  ...
  group_by(jpCourt, noticeConfirm, observer) %>%
  summarize(count = n()) %>%
  mutate(perntv = count/sum(count))
# write.csv(ntvDeacObserver, 'ntvDeacObserver.csv')
# write.csv(ntvDeac, 'data/Notice to Vacate Breakdown.csv')
...
#           observationDate >= as.Date("2024-09-26") & observationDate <= as.Date("2025-01-18")
#          ) %>%
#   group_by(noticeConfirm) %>%
#   summarize(count = n()) %>%
#   mutate(perntv = count/sum(count))
# write.csv(ntvDeacFull, 'data/Notice to Vacate Full.csv')
...
chgevicDeac <-abs(round(((nrow(totevicDeac)-nrow(totevic4))/nrow(totevic4))*100, digits = 1))
chgevicDeac2023 <-abs(round(((nrow(totevicDeac)-nrow(totevic1))/nrow(totevic1))*100, digits = 1))
...
hearing. Observation first began in Dallas County Justice of the Peace Court 1-1 and has since expanded to nearly every JP court.
...
hours. The fall 2024 dataset represents observations by volunteer SMU law students but also includes observations by paid research assistants working in collaboration with the University of Michigan, the University of California San Diego, and the Dallas Eviction Advocacy Center. CPAL staff on-board volunteer observers through a two-part
...
From `r format(min(deac$observationDate, na.rm = TRUE), "%B %d, %Y")` through
`r format(max(deac$observationDate, na.rm = TRUE), "%B %d, %Y")`, 7 SMU law students and 3 research assistants
observed **`r formattable::comma(nrow(deac), digits = 0)`
cases**,
representing **`r paste0(jpCasesCtyDeac$perAllCases*100, "% coverage")`** of all initially scheduled hearings across seven courts. 
Coverage was greatest in `r casesByCourtDeac %>% slice_max(obsCases, n = 1) %>% pull(jpCourt)`: observers recorded data for 2,349
 cases, representing `r scales::percent(max(casesByCourtDeac$perObsCases), accuracy = .1)` coverage. Although observers were in nine courts during the observation period, data from two courts (4-2 and 5-2) are not reported here due to a limited sample of observed cases.
...
subsequently rescheduled for a date later than January 18, 2025, then the case
...
:::
<br>
**Landlords or their representives almost always appeared for their hearings, while tenants appeared about half of the time. **
...
:::
<br>
**Tenants had legal representation in 50% of observed hearings. **
...
them in court. Renters with legal
...
## Across all observed cases, the judge ruled in favor of the landlord in 56% of cases and in favor of the tenant in 10% of cases.
...
## However, appearing in court and having legal representation matters for outcomes. When tenants did not appear for their hearings, cases were decided in favor of the landlord 79% of the time; when tenants did appear, cases were decided in favor of the landlord 53% of the time.
