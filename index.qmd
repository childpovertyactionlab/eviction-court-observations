...
deac <- read_sheet("https://docs.google.com/spreadsheets/d/11X91mVG7XSTDk3fjb8gCiyXDToh80yyvrVdLJiZOpOw/edit?gid=480254842#gid=480254842", col_types = "c") %>%
  ...
  filter(#!jpCourt %in% c("5-2", "3-2", "2-1", '2-2'),
         !is.na(jpCourt),
         observationDate >= as.Date("2024-09-01")
         ) %>%
  mutate(jpCourt = paste("Court", jpCourt),
         observationDate = as.Date(observationDate, format = "%Y-%m-%d"),
...
deac <- deac %>%
  filter(observationDate < "2025-12-01")
...
highchart()%>%
  ...
  hc_add_series(
    ...
    mapping = hcaes(
      ...
      group = legalRep,
      color = ifelse(jpCourt == "Total", c('#ED683F','#4B1707')[match(legalRep, unique(plotDefDeac$legalRep))], palette_cpal_main[match(legalRep, unique(plotDefDeac$legalRep))])
      ),
    dataLabels = list(enabled = TRUE, format = "{point.y:.0f}%", style = list(fontWeight = "bold")))%>%
