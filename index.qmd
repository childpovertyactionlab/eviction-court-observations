...
evictions <- rio::import(paste0(libDB, 'Data Library/Dallas County/Eviction Records/Data/Dallas County Eviction Master/Eviction Cases Test.csv')) %>%
...
and
January 18, 2025,
`r prettyNum(nrow(totevicDeac), big.mark = ',')` evictions were filed in
Dallas County, an average of `r avgevicDeac` per day.** This represents an
`r paste0(abs(chgevic23), "%")` decrease over this same time period in
2023 and a `r paste0(abs(chgevic22), "%")` decrease over the same time
period in 2022.
...
observers recorded data for 2,349 cases, representing
...
Cases are limited to those where the defendant appeared.
...
<br> **Landlords or their representatives almost always appeared for the
hearing, but tenants appeared about half of the time. However, tenant
appearance has improved (+14% points) over previous observation
cycles.**
...
plot_data <- caseAppearDeac

# Unique courts for x-axis
courts <- unique(plot_data$jpCourt)

# Helper to get data in the correct order for highcharter
get_series_data <- function(df, court_order, type_filter, appear_filter) {
  df %>%
    filter(type == type_filter, appear == appear_filter) %>%
    right_join(tibble(jpCourt = court_order), by = "jpCourt") %>%
...
highchart() %>%
  hc_chart(type = "column") %>%
  hc_xAxis(categories = courts) %>%
  hc_plotOptions(series = list(stacking = "percent")) %>%
  hc_add_series(
    name = "Yes (Tenant)",
    data = get_series_data(plot_data, courts, "Tenant (Defendant)", "Yes"),
    stack = "Tenant"
  ) %>%
  hc_add_series(
    name = "No (Tenant)",
    data = get_series_data(plot_data, courts, "Tenant (Defendant)", "No"),
    stack = "Tenant"
  ) %>%
  hc_add_series(
    name = "Unknown (Tenant)",
    data = get_series_data(plot_data, courts, "Tenant (Defendant)", "Unknown"),
    stack = "Tenant"
  ) %>%
  hc_add_series(
    name = "Yes (Landlord)",
    data = get_series_data(plot_data, courts, "Landlord (Plaintiff)", "Yes"),
    stack = "Landlord"
  ) %>%
  hc_add_series(
    name = "No (Landlord)",
    data = get_series_data(plot_data, courts, "Landlord (Plaintiff)", "No"),
    stack = "Landlord"
  ) %>%
  hc_add_series(
    name = "Unknown (Landlord)",
    data = get_series_data(plot_data, courts, "Landlord (Plaintiff)", "Unknown"),
    stack = "Landlord"
  ) %>%
  hc_add_theme(hc_cpal) %>%
  hc_colors(palette_cpal_main) %>%
  hc_tooltip(
    # shared = TRUE,
    pointFormat = "{series.name}</span>: <b>{point.y}</b> ({point.percentage:.1f}%)<br/>",
...
<br> **Legal representation of tenants has improved (+25% points) over
previous observation cycles.**
...
<br> **A written Notice to Vacate is the first legal step to initiate an
eviction and was confirmed by the Justice of the Peace in 53% of
observed hearings, up from 44% in 2022-23.**
...
::: column-margin
Cases marked as "Unknown" for tenant appearance are excluded from this chart.
:::
...
highchart() %>%
  ...
  hc_add_series(data = plotOutApDeac, 
                ...
                hcaes(x = defAppear, 
                      ...
                      group = outcomeParty),
                tooltip = list(pointFormat = "{series.name}: <b>{point.y:.2f}%</b><br>Count: <b>{point.count}</b>")) %>%
  hc_chart(type = "bar") %>%
