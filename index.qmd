...
#2024
totevic4 <- evictions%>%
  filter(filed_date >= as.Date('2023-11-01') & filed_date < as.Date('2024-06-20'))
...
evicMonthstotal <- evicMonthsTime %>%
  group_by(monthFiled, dateMonth) %>%  # Group by month to calculate totals for each month
  summarize(totevic = sum(totevic), yearFiled = "Total")#%>% # Sum totevic and create the "Total" label for yearFiled
  #bind_rows(evicMonthsTime) %>%       # Combine the total rows with the original data
  #arrange(monthFiled) 
...
highchart()%>%
  ...
  hc_title(text = list('Monthly Eviction Totals by Year'))
```
...
been ongoing since, with the purpose of **(1)** improving local
understanding of how eviction unfolds, **(2)** uncovering
barriers/challenges for tenants and landlords, and **(3)** identifying
...
2021, volunteer observers have mostly been SMU Dedman School of Law
...
volunteers expanded to JP Court 1-2 and 4-1 for the first time in Spring
2023, hence the lower coverage rate.
...
highchart()%>%
  ...
  hc_add_series(
    ...
    mapping = hcaes(
      ...
      y = round(perCases, 2),  
      group = type,
      color = ifelse(jpCourt == "Total", c('#4B1707','#ED683F')[match(type, unique(plotCases$type))], palette_cpal_main[match(type, unique(plotCases$type))])
      ),
...
highchart()%>%
  ...
  hc_add_series(plotAppear,
                ...
                mapping = hcaes(
                  ...
                  y = AppearPer,
                  group = type,
                  color = ifelse(jpCourt == "Total", c('#4B1707','#ED683F')[match(type, unique(plotAppear$type))], palette_cpal_main[match(type, unique(plotAppear$type))])
                ),
               dataLabels = list(enabled = TRUE, format = "{point.y:.0f}%", style = list(fontWeight = "bold")))%>%
...
highchart()%>%
  ...
  hc_add_series(
    ...
    mapping = hcaes(
      ...
      group = legalRep,
      color = ifelse(jpCourt == "Total", c('#ED683F','#4B1707')[match(legalRep, unique(plotDef$legalRep))], palette_cpal_main[match(legalRep, unique(plotDef$legalRep))])
      ),
    dataLabels = list(enabled = TRUE, format = "{point.y:.0f}%", style = list(fontWeight = "bold")))%>%
  hc_plotOptions(column = list(borderWidth = 0)) %>%
