...
highchart()%>%
  ...
  hc_colors(cpal_palettes_categorical$main)%>%
  ...
  hc_add_series(plotDeacCases,
                ...
                mapping = hcaes(
                  ...
                  color = ifelse(jpCourt == "Total", 
                                 c('#C3257B','#ED683F')[match(type, unique(plotDeacCases$type))], 
                                 cpal_palettes_categorical$main[match(type, unique(plotDeacCases$type))])),
                color = c('#008097', '#004855'),
...
jpCasesDeac %>%
  ...
  transmute('JP Court' = jpCourt,
    ...
    cpal_table_gt(#options = list(paging = FALSE,
                           # searching = FALSE,
                           # info = FALSE
                           # ),
            # rownames = FALSE
...
evicClean_colors <- c(
  "Holdover/lease expiration" = "#AB8C01",
  "Multiple Reasons" = "#C3257B",
  "Non-payment of rent" = "#004855",
  "Unknown" = "#ED683F",
...
highchart()%>%
  ...
  hc_add_series(
    ...
    mapping = hcaes(
      ...
      group = legalRep,
      color = ifelse(jpCourt == "Total", c('#ED683F','#C3257B', '#F1896A')[match(legalRep, unique(plotDefDeac$legalRep))], cpal_palettes_categorical$main[match(legalRep, unique(plotDefDeac$legalRep))])
      ),
    dataLabels = list(enabled = TRUE, format = "{point.y:.0f}%", style = list(fontWeight = "bold")))%>%
...
outcomeParty_colors <- c(
  "Abated" = "#AB8C01",  
  "Dismissed" = "#C3257B", 
  "Ruled in Favor of Landlord" = "#004855",   
  "Other" = "#798AA1",
...
hc_colors(c(cpal_palettes_categorical$main, "#0047bd"))  %>%
  hc_add_theme(hc_cpal)
